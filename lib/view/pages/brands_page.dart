...
class _BrandsPageState extends BasePageState<BrandsPage> {
  ...
  @override
  void initState() {
    var _productList = Provider.of<CategoriesProvider>(context, listen: false);
    _productList.resetStreams();
    _productList.setLoadingState(LoadMore.INITIAL);
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels ==
        ...
        _productList.setLoadingState(LoadMore.LOADING);
  ...
  Widget _productsList() {
    return new Consumer<CategoriesProvider>(
      builder: (context, model, child) {
        if (model.allProducts != null &&
            model.allProducts.length > 0 &&
            model.getLoadMoreStatus() != LoadMore.INITIAL) {
          return _buildList(model.allProducts,
              model.getLoadMoreStatus() == LoadMore.LOADING);
        }
        return Center(
